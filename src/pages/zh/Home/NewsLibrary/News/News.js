...
import { getZhNews } from 'services/newsService';
...
function News() {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                const [newsData, categoryData] = await Promise.all([getZhNews(), getZhCategoriesBySlug('文章')]);
                setNews(newsData.zhNews);
        ...
        <div className={cx('wrapper')}>
            <div className={cx('inner')}>
                <div className={cx('title-container')}>
                    <Title subText="最新文章" showSeeAll={true} slug={`${routes.newsZH}`} />
                </div>
                {/* <ButtonGroup
                ...
                <div className={cx('news-list')}>
                    {filteredNews.map((news, index) => {
                        ...
                        return (
                            <Link
                                key={index}
                                to={`${routes.newsZH}/${getCategorySlug(news.zh_child_nav_id)}/${news.id}`}
                            >
                                <CardContent
                                    title={news.title}
