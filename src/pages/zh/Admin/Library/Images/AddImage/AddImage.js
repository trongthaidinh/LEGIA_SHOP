...
const AddImage = () => {
    ...
    const handleSubmit = async (values, { resetForm }) => {
        if (files.length === 0) {
            setNotification({ message: 'Vui lòng chọn hình ảnh!', type: 'error' });
            return;
        }
        ...
        try {
            ...
            setNotification({ message: 'Thêm hình ảnh thành công!', type: 'success' });
            resetForm();
            setFiles([]);
            setTimeout(() => {
            ...
            setNotification({ message: 'Có lỗi khi thêm hình ảnh.', type: 'error' });
                ...
                {({ isSubmitting }) => (
                    <Form className={styles.form}>
                        ...
                        <button type="submit" disabled={isSubmitting} className={styles.submitButton}>
                            {isSubmitting ? <div className={styles.spinner}></div> : 'Thêm hình ảnh'}
                        </button>
                    </Form>
                )}
            </Formik>
