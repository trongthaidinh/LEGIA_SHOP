...
import { getZhNavigationLinks } from '~/services/navigationService';
import styles from './Navigation.module.scss';
import Search from '~/layouts/components/ZhSearch';
...
function Navigation({ isFixed }) {
    ...
    useEffect(() => {
        const fetchNavigationLinks = async () => {
            try {
                const links = await getZhNavigationLinks();
    ...
    useEffect(() => {
        ...
        window.addEventListener('zhCartUpdated', handleCartUpdate);

        return () => {
            window.removeEventListener('zhCartUpdated', handleCartUpdate);
        };
    }, []);
                    ...
                    <div className={cx('navigation-utilities')}>
                        ...
                        <div className={cx('right-section')}>
                            ...
                            <div className={cx('cart')}>
                                <Link to={routes.cartZH} className={cx('cart-link')}>
                                    <FontAwesomeIcon icon={faBasketShopping} className={cx('icon')} />
                                    <div className={cx('text')}>
                                        <span className={cx('cart-items-text')}>购物车</span>
                                        <span className={cx('cart-items')}>
                                            {cartQuantity} <span className={cx('cart-items-text')}>產品</span>
                                        </span>
                                    </div>
                                </Link>
                            </div>
                    ...
                    <ul className={cx('navigation-links', { open: isMenuOpen })}>
                        <li onClick={handleLinkClick}>
                            <div className={cx('menu-item')}>
                                <NavLink end to="/zh" className={({ isActive }) => cx({ 'active-link': isActive })}>
                                    <div className={cx('item-icon')}>首页</div>
                                </NavLink>
                            </div>
                        </li>
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className={cx('sub-link-wrapper')}>
                                                            <NavLink
                                                                to={`${routes.homeZH}/${link.slug}/${childLink.slug}`}
                                                                // to={`/${link.slug}`}
                                                                className={({ isActive }) =>
