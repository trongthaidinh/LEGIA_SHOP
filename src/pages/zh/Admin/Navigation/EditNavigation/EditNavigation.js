...
import {
    getZhMainNavigationById,
    getZhSubNavigationById,
    getZhChildNavigationById,
    updateZhMainNavigationLink,
    updateZhSubNavigationLink,
    updateZhChildNavigationLink,
    getZhNavigationLinks,
...
const EditNavigation = () => {
    ...
    const fetchNavigation = async () => {
        ...
        try {
            let data;
            if (type === 'main') {
                data = await getZhMainNavigationById(id);
            } else if (type === 'sub') {
                data = await getZhSubNavigationById(id);
            } else if (type === 'child') {
                data = await getZhChildNavigationById(id);
            } else {
    ...
    useEffect(() => {
        const fetchNavigations = async () => {
            setLoading(true);
            try {
                const data = await getZhNavigationLinks();
                setNavigations(data);
            } catch (error) {
                setIsError(true);
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (type === 'main') {
                await updateZhMainNavigationLink(id, { title, position });
            } else if (type === 'sub') {
                await updateZhSubNavigationLink(id, { title, position });
            } else if (type === 'child') {
                await updateZhChildNavigationLink(id, { title, position });
            } else {
                throw new Error('Invalid navigation type');
            }
            ...
            setTimeout(() => {
                navigate(routes.navigationListZH);
            }, 1000);
        } catch (error) {
