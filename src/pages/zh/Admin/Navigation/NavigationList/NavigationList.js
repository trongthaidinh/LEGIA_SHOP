...
import {
    ...
    deleteZhMainNavigationLink,
    deleteZhSubNavigationLink,
    deleteZhChildNavigationLink,
...
const NavigationList = () => {
    ...
    const handleDelete = async (type, id) => {
        if (window.confirm('Bạn có chắc muốn xóa navigation này không?')) {
            try {
                if (type === 'main') {
                    await deleteZhMainNavigationLink(type, id);
                } else if (type === 'sub') {
                    await deleteZhSubNavigationLink(type, id);
                } else if (type === 'child') {
                    await deleteZhChildNavigationLink(type, id);
                }
    ...
    return (
        <div className={styles.navigationContainer}>
            ...
            <div className={styles.actionsContainer}>
                ...
                />
                <Link to={routes.addNavigationZH} className={styles.addButton}>
                            ...
                            currentNavigations.map((nav) => (
                                <React.Fragment key={nav.id}>
                                    <tr>
                                        ...
                                        <td>
                                            <Link
                                                to={`/zh/admin/edit-navigation/${nav.id}?type=${nav.type}`}
