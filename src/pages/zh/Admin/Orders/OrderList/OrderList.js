...
import { getZhOrders, updateZhStatus } from 'services/orderService';
...
const OrderList = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const orderData = await getZhOrders();
                setOrders(orderData);
            } catch (error) {
    ...
    const handleCancelOrder = async (orderId) => {
        AntdModal.confirm({
            ...
            onOk: async () => {
                ...
                try {
                    await updateZhStatus(orderId, { status: 'canceled' });
    ...
    const handleStatusChange = async (orderId, newStatus) => {
        setLoadingOrderId(orderId);
        try {
            await updateZhStatus(orderId, { status: newStatus });
            setOrders((prevOrders) =>
