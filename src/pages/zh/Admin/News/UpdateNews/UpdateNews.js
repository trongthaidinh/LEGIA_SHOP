...
import { getZhNewsById, updateZhNews } from '~/services/newsService';
...
const UpdateNews = () => {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const fetchedCategories = await getZhCategoriesBySlug('文章');
        ...
        const fetchNews = async () => {
            try {
                const news = await getZhNewsById(id);
                setInitialValues({
                    title: news.title,
                    summary: news.summary,
                    image: news.images[0],
                    categoryId: news.zh_child_nav_id,
